...
public class UDPServer {
	...
	boolean stop;
	DatagramSocket dataSocket;    //연결할때
	DatagramPacket receivePacket; //데이터담는거(우편봉투역할)

	public void serverStart() throws SocketException {
		// DatagramPacket 수신/송신을 위한 DatagramSocket 생성
		dataSocket = new DatagramSocket(PORT);  //생성자체가 서버구동한거!
		System.out.println("### UDP EchoServer Start!! ###");
		while (!stop) {
			// 클라이언트 데이터 수신을 위한 DatagramPacket 생성(빈우편물)
			byte[] buffer = new byte[512];								//버퍼로받음! 우편물받기위한 통
			receivePacket = new DatagramPacket(buffer, buffer.length); //우편물
