...
public class UDPClient {
	...
	public void clientStart() throws Exception{
		...
		//클라이언트에서 지정한 포트는 받을때 사용

		String inputMessage = "안녕하세요. 김기정입니다..";
		byte[] buffer = inputMessage.getBytes(); //인코딩과정
		
		// 서버에 전송할 우편물 생성
		sendPacket = new DatagramPacket(buffer, buffer.length, InetAddress.getByName(serverIP), 2018); //받는사람주소, 받을포트번호
		
		// 서버에 데이터(우편물) 전송
		dataSocket.send(sendPacket);   //확인절차없음-->빨라
